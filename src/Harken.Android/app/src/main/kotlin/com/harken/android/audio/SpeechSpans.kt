...
object SpeechSpans {
     ...
     */
    const val MIN_SKIPPABLE_SILENCE_SECONDS = 10
     ...
     */
    const val MAX_SPAN_SECONDS = 300
     ...
     */
    const val MIN_AMPLITUDE_THRESHOLD = 60
     ...
     */
    const val MAX_AMPLITUDE_THRESHOLD = 500
     ...
     * [MAX_AMPLITUDE_THRESHOLD] is right for the second and far too high for the first: a
     ...
     * invented the "you"s.
     *
     * Takes one RMS reading per [WINDOW_SECONDS] of [totalSamples] rather than the audio
    ...
    fun assemble(
        windowRms: IntArray,
        totalSamples: Int,
        sampleRate: Int = WavFormat.SAMPLE_RATE,
    ): List<SpeechSpan> {
        require(sampleRate > 0) { "sampleRate must be positive" }
        if (totalSamples <= 0) return emptyList()

        val threshold = amplitudeThreshold(windowRms)
        val windowSamples = sampleRate * WINDOW_SECONDS
        val padding = sampleRate * PADDING_SECONDS
        val maxSilentGap = sampleRate * MIN_SKIPPABLE_SILENCE_SECONDS
        ...
        for (window in windowRms.indices) {
            ...
            // A gap shorter than MIN_SKIPPABLE_SILENCE_SECONDS is a pause, not a hole:
     ...
     */
    private fun SpeechSpan.grownTo(
        minSamples: Int,
        totalSamples: Int,
    ): SpeechSpan {
