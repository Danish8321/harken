...
class SessionRepository(
    ...
    private companion object {
        ...
        const val TITLE_MATCH_LIMIT = 50
    }
     ...
     * see [SearchQuery.SEGMENT_MATCH_LIMIT].
     */
    ...
    /**
     * Every recording, oldest first, formatted for a backup. [displayTitle] resolves the
    ...
    suspend fun rename(
        id: UUID,
        title: String?,
    ) = dao.setTitle(id, title?.trim()?.ifBlank { null })
     ...
     * [SearchQuery.MIN_LENGTH] returns nothing rather than most of the database.
     */
    suspend fun search(query: String): List<SearchHit> {
        val term = query.trim()
        if (term.length < SearchQuery.MIN_LENGTH) return emptyList()

        val startNanos = System.nanoTime()
        val pattern = SearchQuery.likePattern(term)
        val matches = dao.searchSegments(pattern, SearchQuery.SEGMENT_MATCH_LIMIT)
        val titleRows = dao.searchTitles(pattern, TITLE_MATCH_LIMIT)
     ...
     * straight back.
     */
    suspend fun purge(id: UUID): Result<Unit> =
        runCatching {
    ...
    private fun toView(row: SessionRow) =
        SessionView(
            id = row.id,
